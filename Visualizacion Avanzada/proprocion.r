...
setwd("C:/Users/Bea/Downloads/uned")
...
#pie charts
# Cálculo de la proporción de casos por grupo de edad y por provincia
prop_edad <- covid_2020 %>% 
  group_by(grupo_edad) %>% 
  summarize(total_casos = sum(casos_prop)) %>% 
  mutate(prop_casos = total_casos/sum(total_casos))

prop_prov <- covid_2020 %>% 
  group_by(provincia_iso) %>% 
  summarize(total_casos = sum(casos_prop)) %>% 
  mutate(prop_casos = total_casos/sum(total_casos))

# Creación de los pie charts
pie_edad <- ggplot(prop_edad, aes(x = "", y = prop_casos, fill = grupo_edad)) +
  geom_bar(width = 1, stat = "identity") +
  coord_polar(theta = "y") +
  labs(fill = "Grupo de edad", x = NULL, y = NULL) +
  theme_void()

pie_prov <- ggplot(prop_prov, aes(x = "", y = prop_casos, fill = provincia_iso)) +
  geom_bar(width = 1, stat = "identity") +
  coord_polar(theta = "y") +
  labs(fill = "Provincia", x = NULL, y = NULL) +
  theme_void()

# Visualización de los pie charts
pie_edad
pie_prov
